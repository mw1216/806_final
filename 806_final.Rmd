...
select(database, Month, Year, Temperature) %>%
  ...
  "Year", x = "Month", y = "Temperature (C)") + scale_color_brewer(palette = "Dark2") + scale_x_continuous(breaks=c(6, 7, 8)) -> temp_graph
...
select(database, Month, Year, DO) %>%
  ...
  as.factor(Year),
  group = Year, by = Year)) + geom_point() + geom_line() + labs(color = 
  "Year", x = "Month", y = "Dissolved Oxygen") + scale_color_brewer(palette = "Dark2") + scale_x_continuous(breaks=c(6, 7, 8)) -> DO_graph
...
select(database, Month, Year, pH) %>%
  ...
  as.factor(Year),
  group = Year)) + geom_point() + geom_line() + labs(color = 
  "Year", x = "Month", y = "pH") + scale_color_brewer(palette = "Dark2") + scale_x_continuous(breaks=c(6, 7, 8)) -> pH_graph

```

```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(ggpubr)
ggarrange(pH_graph, temp_graph, DO_graph, labels = c("A", "B", "C"),
  ncol = 2, nrow = 2)
...
select(database, Depth, DO, Month, Year) %>%
  ...
  group_by(Depth) %>%
  summarize(mean_DO = mean(DO)) %>%
  ggplot(mapping = aes(x = Depth, y = mean_DO)) + geom_point() + geom_line() + labs(x = "Depth", y = "Dissolved Oxygen")
...
select(database, Phylum, value, Month, Year, Temperature) %>%
  filter(Year == 2009, value > 100, Temperature != "NA", Month != 5) %>%
...
select(database, Phylum, value, Year, Month) %>%
  filter(value > 200, Year == 2007, Month == 6 | Month == 7 | Month == 8) %>%
  group_by(Year, Phylum) %>%
  summarize(value=sum(value)) %>%
  ggplot(mapping = aes(x = Phylum, y = 
  value)) + geom_bar(stat = "identity", fill = "steelblue") + ggtitle("2007") + labs(x = "Phylum", y = "Abundance") + theme(axis.text.x = element_text(angle = 90, vjust =
  0.5)) -> graph_1
...
select(database, Phylum, value, Year, Month) %>%
  filter(value > 200, Year == 2009, Month == 6 | Month == 7 | Month == 8) %>%
  group_by(Year, Phylum) %>%
  summarize(value=sum(value)) %>%
  ggplot(mapping = aes(x = Phylum, y = 
  value)) + geom_bar(stat = "identity", fill = "Coral2") + ggtitle("2009") + labs(x =
  "Phylum", y = "Abundance") + theme(axis.text.x = element_text(angle = 90, vjust =
  0.5))-> graph_2
```
...
select(database, Phylum, value, pH, Year) %>%
  ...
  Phylum, by = Phylum)) + geom_bar(position="stack", stat="identity") +
