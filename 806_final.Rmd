...
```
# remove at end but save for code reference
```{r, echo = FALSE}
  theme(legend.text = element_text(size = 6), 
  legend.key.size = unit(0.35, "cm"), 
  legend.key.height = unit(0.35, "cm"),  
...
select(database, Phylum, value, Year, Month) %>%
  ...
  value)) + geom_bar(stat = "identity", fill = "steelblue") + ggtitle("Most Abundant Phyla: Summer 2007") + labs(x = "Phylum", y = "Abundance") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
select(database, Phylum, value, Year, Month) %>%
  ...
  summarize(value=sum(value)) %>%
  ggplot(mapping = aes(x = Phylum, y = 
  value)) + geom_bar(stat = "identity", fill = "Coral2") + ggtitle("Most Abundant Phyla: Summer 2009") + labs(x = "Phylum", y = "Abundance")
```
